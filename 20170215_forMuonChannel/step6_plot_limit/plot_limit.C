...
void plot_limit(){
  ...
  // -------------------------------------------------------
  // (1) access the limit (in signal strength) in ROOT file



  vector<double> v_mh, v_median, v_68l, v_68h, v_95l, v_95h, v_obs;
  v_mh .clear(); v_median .clear(); v_68l .clear(); v_68h .clear(); v_95l .clear(); v_95h .clear(); v_obs. clear();

  int Xmass[nXm]={800,1000,1200,1400,1600,1800,2000,2500,3000,3500,4000};

  for(int n=0;n<nXm;n++){// loop for mass points

    char file_name[200]; sprintf( file_name ,"limit_result/higgsCombineTest.Asymptotic.mH%d.root",Xmass[n]);

    TFile *file = new TFile( file_name , "READ"); cout<<" Read limit file: "<< file_name <<endl;
    TTree *tree = (TTree*) file->Get("limit");

    double mh, limit; float quant;

    tree->SetBranchAddress("mh", &mh);                  // mass point
    tree->SetBranchAddress("limit", &limit);            // limit in signal strength
    tree->SetBranchAddress("quantileExpected", &quant); // for different kinds of limits

    bool Once = true; 

    for (int i = 0; i < tree->GetEntries(); i++) {// loop to access entries

      tree->GetEntry(i);
      
      if(Once == true) { v_mh.push_back(mh); Once = false;} // Save mass point only once

      cout<<" quant : "<<quant<<" limit : " <<limit<<endl;

      if      (quant > -1.01  && quant < -0.99) { v_obs.push_back(limit);   } // observed 
      else if (quant >  0.02  && quant <  0.03) { v_95l.push_back(limit);   } // 95low
      else if (quant >  0.15  && quant <  0.17) { v_68l.push_back(limit);   } // 68low
      else if (quant >  0.49  && quant <  0.51) { v_median.push_back(limit);} // expected
      else if (quant >  0.83  && quant <  0.85) { v_68h.push_back(limit);   } // 68high
      else if (quant >  0.965 && quant <  0.98) { v_95h.push_back(limit);   } // 95high
      else {  cout << "Error! Quantile =  " << quant << endl;}

    }// end entries loop

  }// end mass points loop


/*
  cout<<" test "<< endl;
  for(int n=0;n<nXm;n++){ 
    cout<<"n: "<< n << endl; cout<<"mass point: "<< v_mh.at(n) << endl; 
    cout<<"central value of expected limit: "<< v_median.at(n) << endl; 
  }
*/
  ...
  // -------------------------------------------------------
  // (2) access the theory from text file

  string xsec_file= "./13TeV_xsec_Zh.txt";

  ifstream open_txt( xsec_file.c_str(), ios::in);
  if (! open_txt.is_open()) { cout << "Failed to open file with xsections: " << xsec_file << endl;}

  float mH, CS; 
  vector<float> v_mhxs, v_xs ; 
  v_mhxs .clear(); v_xs .clear();

  while (open_txt.good()) { 
    open_txt >> mH >> CS; 
    v_mhxs.push_back(mH); v_xs.push_back(CS); 
  }
...
/*
  cout<<" test "<< endl;
  for(int n=0;n<v_mhxs.size();n++){ 
    cout<<"n: "<< n << endl; 
    cout<<"v_mhxs.at(n): "<< v_mhxs.at(n) << endl; 
    cout<<"v_xs.at(n): "  << v_xs.at(n)   << endl; 
  }
*/
  ...
  // -------------------------------------------------------
  // (3) plot in TGraphAsymmErrors


  double mass[nXm], medianD[nXm];

  for (int im = 0; im < nXm; im++) {

    mass[im] = Xmass[im];
    double fl_xs = double ( v_xs.at(im) );
    medianD[im] = v_median.at(im) * fl_xs; 

  }

  TGraphAsymmErrors*  grmedian_cls = new TGraphAsymmErrors( nXm , mass, medianD);
  ...
  double fr_left = 700.0, fr_down =5.0E-2, fr_right = 4100, fr_up = 1E+3;
  ...
  grmedian_cls->SetLineColor(2);
  grmedian_cls->SetLineStyle(1);
  grmedian_cls->SetLineWidth(3);
  grmedian_cls->Draw();

  c1->Print("ExpectedLimit_in_Muon_channel.pdf");  
...
//const string dirLimitFile = "./limit_package_output/";
...
cout<<"file n:"<< n << endl;
cout<<"v_obs .size()"<<v_obs .size()<<endl;
cout<<"v_median .size()"<< v_median .size()<<endl;
...
string xsect_file_th = dirXSect + "13TeV_xsec_Zh.txt";
  ...
  v_mhxs .clear(); v_xs .clear(); v_toterrh .clear(); v_toterrl .clear();
...
//  for(int i ; i<v_mhxs.size() ; i++){
//	cout<<"v_mhxs[i]: "<< v_mhxs[i]<<endl;
//	cout<<"v_xs[i]:"<< v_xs[i] <<endl;
//  }
  ///////////////////////////
...
cout<<"nXm:"<< nXm << endl;
cout<<"v_xs .size():"<< v_xs .size() << endl;
cout<<"v_obs.size():"<< v_obs.size() << endl;
cout<<"v_median .size():"<<v_median .size()  << endl;  

  for (int im = 0; im < nXm; im++) {

    cout<<"nMassEff:" << nMassEff << endl;
//    cout<<"v_xs.at(im): "<< v_xs.at(im) << endl;
...
// / *      up68err[nMassEff] = (v_68h.at(im) - v_median.at(im)) * fl_xs;
      ...
      xs10[nMassEff] = fl_xs10; // times 100.0;
