...
void Fit_TwoRange(TH1F* &histo1D,
  ...
  cout<<endl<<"Fit in ["<< Xmin1 <<","<< Xmax1<<"] + ["<< Xmin2<<","<<Xmax2<<"]"<<endl<<endl; 
...
//  xmin2= 30; xmax1=200; reject = false;// draw signal region
  ...
  if( fn_name =="Quadric" ){
    ...
    fnLeft  = new TF1("fnLeft" , fline_Quadric ,50,200,3);
    fnRight = new TF1("fnRight", fline_Quadric ,50,200,3);

    fn_extend = new TF1("fn_extend", fline_Quadric ,50,200,3);
  }
  ...
  if( fn_name =="ExpTail" ){
    ...
    fnLeft    = new TF1("fnLeft" ,   fline_ExpTail ,50,200,3);
    fnRight   = new TF1("fnRight",   fline_ExpTail ,50,200,3);

    fn_extend = new TF1("fn_extend", fline_ExpTail ,50,200,3);
...
*/
  ...
  fn_extend ->SetParameters(fn->GetParameters());


  fnLeft    ->SetRange( xmin1 , ymin , xmax1 , ymax );
  fnRight   ->SetRange( xmin2 , ymin , xmax2 , ymax );

  fn_extend ->SetRange( xmax1 , ymin , xmin2 , ymax );

  reject = false;// display fn_extend in extended region

  fn_extend->SetLineColor(416); // green
//  fnLeft   ->SetLineColor(4);   // blue
//  fnRight  ->SetLineColor(6);   // pink  
...
/*
  // test in reject region
  reject = true;
  double Area_fit   = fnRight->Integral(100,120);
  cout<<"test in reject region: "<<Area_fit << endl;

  reject = false;

  Area_fit   = fnRight->Integral(100,120);
  cout<<"test in reject region: "<<Area_fit << endl;
...
cout<<"histo1D->GetMaximum():"<< histo1D->GetMaximum() << endl;
...
/*
void make_RatioPad(TCanvas* &c,TPad* &pad,TH1F* h_above,TH1F* h_below ){

c->cd();

pad  ->SetTopMargin(0.9);
pad  ->SetBottomMargin(0.3);
pad  ->Draw();
pad  ->cd();

TH1F* h_ratio = (TH1F*) h_above ->Clone("h_ratio"); 
h_ratio  ->Sumw2();
h_ratio  ->Divide( h_below );
